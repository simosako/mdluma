...
global.process = originalProcess;
    ...
    #[test]
    fn keyboard_shortcut_ctrl_e_requests_external_editor_once() {
        let assets = EmbeddedUiAssets::default();
        let script = assets
            .read_text_asset(UiTextAsset::AppJs)
            .expect("read app js");
...
const calls = [];
const originalProcess = global.process;
let externalEditorClicks = 0;

const externalEditorItem = {
  disabled: false,
  click() {
    externalEditorClicks += 1;
  },
};
...
global.document = {
  ...
  querySelector(selector) {
    ...
    if (selector === '[data-action="external-editor"]') {
      return externalEditorItem;
    }
    return null;
  },
};
...
if (externalEditorClicks !== 1) {
  throw new Error("ctrl+e should click external editor action once, got " + externalEditorClicks);
}

if (calls.length !== 0) {
  throw new Error("ctrl+e should use existing external editor action path, got xcalls: " + JSON.stringify(calls));
}

if (prevented !== 1) {
  throw new Error("ctrl+e should prevent default once, got " + prevented);
}
